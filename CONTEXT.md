...
| **ProductCatalouge** | Java/Jooby + Vue e-commerce app | Live: `productcatalouge.onrender.com` | Deployed under ticket 14 — it previously needed a hand-started H2 server and shipped no data, so it now carries a seeded in-memory database and a Dockerfile. Free instance, sleeps when idle (first load can take ~1 min); demo account `demo`/`demo1234`. Solo work on a staff-provided starter scaffold (Gradle/Vue vendor files only). |
